...
if config.log_to_wandb:
    #Set wandb to log the training process # entity = config.entity, 
...
env = make_atari_env(config.env_id, n_envs=config.n_envs, seed=config.seed) # NoFrameskip
...
if config.transfer_type == "CNN_Q-net":
    print("all networks transfered.")
    pretrained_cnn_weights = source_model.q_net.features_extractor.cnn.state_dict()
    model.q_net.features_extractor.cnn.load_state_dict(pretrained_cnn_weights)

elif config.transfer_type == "CNN_Q-net-target":
    pretrained_cnn_weights = source_model.q_net_target.features_extractor.cnn.state_dict()
    model.q_net_target.features_extractor.cnn.load_state_dict(pretrained_cnn_weights)


elif config.transfer_type == "CNN-all":
    pretrained_cnn_weights = source_model.q_net.features_extractor.cnn.state_dict()
    pretrained_cnn_weights_target = source_model.q_net_target.features_extractor.cnn.state_dict()
    model.q_net_target.features_extractor.cnn.load_state_dict(pretrained_cnn_weights_target)
    model.q_net.features_extractor.cnn.load_state_dict(pretrained_cnn_weights)


elif config.transfer_type == "CNN_LINEAR_Q-net":
    pretrained_cnn_weights = source_model.q_net.features_extractor.cnn.state_dict()
    pretrained_linear_weights = source_model.q_net.features_extractor.linear.state_dict()
    model.q_net.features_extractor.cnn.load_state_dict(pretrained_cnn_weights)
    model.q_net.features_extractor.linear.load_state_dict(pretrained_linear_weights)
