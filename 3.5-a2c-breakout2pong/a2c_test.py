...
if config.log_to_wandb:
    wandb.init(project=config.name_test,sync_tensorboard=config.sync_tensorboard, name=config.name_test, notes=config.notes)
...
if config.pretrained:
    # model = A2C.load(model_path, custom_objects=custom_objects, device=args.device, **kwargs)
    ...
    # model.policy.load_state_dict(torch.load(os.path.join(config.unzip_file_path, "policy.pth"), weights_only = True))
    # model.policy.optimizer.load_state_dict(torch.load(os.path.join(config.unzip_file_path, "policy.optimizer.pth"), weights_only = True))
'''
